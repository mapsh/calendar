...
class MonthView extends View {
    ...
    protected static       int DEFAULT_HEIGHT        = 48;                           // 默认一行的高度
...
//    private boolean mIsDisplayTag;                                    // 是否显示标签
    ...
    private List<MonthAdapter.CalendarDay> mInvalidDays;          // 禁用的日期
    private List<MonthAdapter.CalendarDay> mBusyDays;             // 被占用的日期
    private MonthAdapter.CalendarDay       mNearestDay;                 // 比离入住日期大且是最近的已被占用或者无效日期
    private List<MonthAdapter.CalendarDay> mCalendarTags;         // 日期下面的标签
     ...
     * 被选中日期区间两端 Paint
     */
    protected Paint mSelectedDayBothEndsBgPaint;
    ...
    MonthAdapter.CalendarDay cellCalendar;        // cell的对应的日期
     ...
     * @param dataModel
     */
    public MonthView(Context context, TypedArray typedArray, DayPickerView.DataModel dataModel) {
        super(context);
        ...
        mCurrentDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorCurrentDay, resources.getColor(R.color.calendar_normal_day));
        mYearMonthTextColor = typedArray.getColor(R.styleable.DayPickerView_colorYearMonthText, resources.getColor(R.color.calendar_normal_day));
...
//        mDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorDayName, resources.getColor(R.color.normal_day));
        ...
        mBusyDaysTextColor = typedArray.getColor(R.styleable.DayPickerView_colorBusyDaysText, resources.getColor(R.color.calendar_normal_day));
        mInValidDaysTextColor = typedArray.getColor(R.styleable.DayPickerView_colorInValidDaysText, resources.getColor(R.color.calendar_normal_day));
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);

        mStringBuilder = new StringBuilder(50);

        MINI_DAY_NUMBER_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeDay, resources.getDimensionPixelSize(R.dimen.calendar_text_size_day));
        TAG_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeTag, resources.getDimensionPixelSize(R.dimen.calendar_text_size_tag));
        YEAR_MONTH_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeYearMonth, resources.getDimensionPixelSize(R.dimen.calendar_text_size_month));
        WEEK_TEXT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_textSizeWeek, resources.getDimensionPixelSize(R.dimen.calendar_text_size_day_name));
        MONTH_HEADER_SIZE = typedArray.getDimensionPixelOffset(R.styleable.DayPickerView_headerMonthHeight, resources.getDimensionPixelOffset(R.dimen.calendar_header_month_height));
        DAY_SELECTED_RECT_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_selectedDayRadius, resources.getDimensionPixelOffset(R.dimen.calendar_selected_day_radius));

        mRowHeight = ((typedArray.getDimensionPixelSize(R.styleable.DayPickerView_calendarHeight,
                resources.getDimensionPixelOffset(R.dimen.calendar_view_height)) - MONTH_HEADER_SIZE - ROW_SEPARATOR) / 6);
        ...
        cellCalendar = new MonthAdapter.CalendarDay();
    ...
    private void onDayClick(MonthAdapter.CalendarDay calendarDay) {
        if (mOnDayClickListener != null && (isPrevDayEnabled || !beforeToday(calendarDay.day, today))) {
     ...
     * @return
     */
    private boolean beforeToday(int monthDay, Calendar time) {
     ...
     * @param canvas
     */
    protected void drawMonthDay(Canvas canvas) {
        // ?
        int y = MONTH_HEADER_SIZE + ROW_SEPARATOR + mRowHeight / 2;
        //列宽
        int rowWidth = mWidth / mNumDays;
        int paddingDay = (mWidth / mNumDays);
        int dayOffset = findDayOffset();
        int day = 1;

        while (day <= mNumCells) {
            int x = rowWidth * (dayOffset) + (int) (0.5 * rowWidth);
            ...
            if (!isPrevDayEnabled && beforeToday(day, today)) {
                isPrevDay = true;
                mDayTextPaint.setColor(mPreviousDayTextColor);
                canvas.drawText(String.format(Locale.CHINA, "%d", day), x, getTextYCenter(mDayTextPaint, y), mDayTextPaint);
            }
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                ...
                drawDayLeftIntervalBg(canvas, x, y);
            ...
            if (mEndDate != null && cellCalendar.equals(mEndDate) && !mStartDate.equals(mEndDate)) {
                ...
                drawDayBg(canvas, x, y, mSelectedDayBgPaint);
                mDayTextPaint.setColor(mSelectedDayTextColor);
                canvas.drawText("离店", x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
            }
...
//                canvas.drawText(String.format("%d", day), x, y - DAY_SELECTED_RECT_SIZE / 2, mDayTextPaint);
            ...
            for (MonthAdapter.CalendarDay calendarDay : mBusyDays) {
...
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
            ...
            for (MonthAdapter.CalendarDay calendarDay : mInvalidDays) {
...
//                    canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
            ...
            if (!isPrevDay && !isInvalidDays && !isBusyDay && !isBeginDay && !isLastDay) {
                ...
                for (MonthAdapter.CalendarDay calendarDay : mCalendarTags) {
     ...
     * @return
     */
    public MonthAdapter.CalendarDay getDayFromLocation(float x, float y) {
        ...
        MonthAdapter.CalendarDay calendar = new MonthAdapter.CalendarDay(mYear, mMonth, day);

        // 获取日期下面的tag
        boolean flag = false;
        for (MonthAdapter.CalendarDay calendarTag : mCalendarTags) {
...
//        mTitleBGPaint.setStyle(Style.FILL);
        ...
        // 被选中的日期左右区间背景paint
        mSelectedDayBothEndsBgPaint = new Paint();
        mSelectedDayBothEndsBgPaint.setFakeBoldText(true);
        mSelectedDayBothEndsBgPaint.setAntiAlias(true);
        mSelectedDayBothEndsBgPaint.setColor(getContext().getResources().getColor(R.color.date_range_picker_selected_both_ends));
        mSelectedDayBothEndsBgPaint.setTextAlign(Align.CENTER);
        mSelectedDayBothEndsBgPaint.setStyle(Style.FILL);
        mSelectedDayBothEndsBgPaint.setAlpha(SELECTED_CIRCLE_ALPHA);

        // 被选中的日期背景paint
        mSelectedDayBgPaint = new Paint();
        mSelectedDayBgPaint.setFakeBoldText(true);
        mSelectedDayBgPaint.setAntiAlias(true);
        mSelectedDayBgPaint.setColor(Color.parseColor("#CFF2EE"));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            MonthAdapter.CalendarDay calendarDay = getDayFromLocation(event.getX(), event.getY());
//            boolean isValidDay = false;
            if (calendarDay == null) {
                return true;
            }

            for (MonthAdapter.CalendarDay day : mInvalidDays) {
            ...
            for (MonthAdapter.CalendarDay day : mBusyDays) {
                // 选择了入住日期，这时候比入住日期大且离入住日期最近的不可用日期变为可选
                if (calendarDay.equals(day) && !(mEndDate == null && mNearestDay != null && calendarDay.equals(mNearestDay))) {
                    return true;
                }
    ...
    public void setMonthParams(HashMap<String, Object> params) {
        ...
        if (params.containsKey(VIEW_PARAMS_SELECTED_BEGIN_DATE)) {
           // mStartDate = (MonthAdapter.CalendarDay) params.get(VIEW_PARAMS_SELECTED_BEGIN_DATE);
        }
        if (params.containsKey(VIEW_PARAMS_SELECTED_LAST_DATE)) {
          //  mEndDate = (MonthAdapter.CalendarDay) params.get(VIEW_PARAMS_SELECTED_LAST_DATE);
        }

        if (params.containsKey("mNearestDay")) {
            mNearestDay = (MonthAdapter.CalendarDay) params.get("mNearestDay");
        }
    ...
    private void drawDayBg(Canvas canvas, int x, int y, Paint paint) {
        RectF rectF = new RectF(x - mWidth / 14, y - 100,
                x + mWidth / 14, y + 100);
     ...
     * @param y
     */
    private void drawDayLeftIntervalBg(Canvas canvas, int x, int y) {
        RectF rectF = new RectF(x - mWidth / 14, y - 100,
                x + mWidth / 14, y + 100);

        float[] radii = {15f, 15f, 0f, 0f, 0f, 0f, 15f, 15f};
        Path path = new Path();
        path.addRoundRect(rectF, radii, Path.Direction.CW);

        canvas.drawPath(path, mSelectedDayBothEndsBgPaint);
    }
     ...
     * @param y
     */
    private void drawDayRightIntervalBg(Canvas canvas, int x, int y) {
        RectF rectF = new RectF(x - mWidth / 14, y - 100,
                x + mWidth / 14, y + 100);

        float[] radii = {0f, 0f, 15f, 15f, 15f, 150f, 0f, 0f};
        Path path = new Path();
        path.addRoundRect(rectF, radii, Path.Direction.CW);
     ...
     *
     * @param paint 画笔
     * @param y     单元格1/2高度相对于屏幕的像素值
     * @return
     */
    private float getTextYCenter(Paint paint, int y) {
        Paint.FontMetrics fm = paint.getFontMetrics();
        return y - fm.descent + (fm.bottom - fm.top) / 2;
    }
}